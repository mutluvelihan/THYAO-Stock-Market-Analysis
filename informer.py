...
informer_model = InformerModel(input_size=input_dim, hidden_size=hidden_dim, output_size=output_dim, num_layers=3, dropout=0.4)
...
for epoch in range(epochs):
    ...
    informer_outputs = informer_model(X_train_tensor.unsqueeze(1))
    informer_loss = criterion(informer_outputs, y_train_tensor)
    informer_loss.backward()
    informer_optimizer.step()
    informer_train_losses.append(informer_loss.item())

    informer_model.eval()
    with torch.no_grad():
        informer_test_outputs = informer_model(X_test_tensor.unsqueeze(1))
        informer_test_loss = criterion(informer_test_outputs, y_test_tensor)
        informer_test_losses.append(informer_test_loss.item())
    informer_model.train()
...
informer_future_predictions = scaler.inverse_transform(informer_future_predictions.reshape(-1, 1)).flatten()
...
plt.plot(future_dates, informer_future_predictions, label="Informer Tahminleri", linestyle="dashed", color="blue")

plt.legend()
plt.xlabel("Tarih")
plt.ylabel("Fiyat")
plt.title("Informer Tahmin Grafiği")
plt.grid(True)
plt.show()
...
rmse = np.sqrt(mse)
r2 = r2_score(y_test_actual, informer_future_predictions)

print("Informer Model Performansı:")
print(f"MSE: {mse}")
print(f"MAPE: {mape}")
print(f"MAE: {mae}")
print(f"RMSE: {rmse}")
